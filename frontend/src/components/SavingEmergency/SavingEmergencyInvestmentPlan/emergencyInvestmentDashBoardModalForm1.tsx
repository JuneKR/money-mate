...
> = ({ title, savingEmergency, savingInvestmentPort }) => {
  ...
  const [investTargetAmount, setInvestTargetAmount] = useState("");
  const [investTimePeriod, setInvestTimePeriod] = useState("");
  const [investTimeRemaining, setInvestTimeRemaining] = useState("");
  const [investMonthlySaving, setInvestMonthlySaving] = useState("");
  const [investRiskSpectrum, setInvestRiskSpectrum] = useState("");
  const [investReturnRate, setInvestReturnRate] = useState("");
  ...
  const updateEmergencyPlan2 = async () => {
  ...
  const updateInvestmentData = async () => {
    const updateInvestmentDataPortData = {
      risk_spectrum: investRiskSpectrum,
      return_rate: investReturnRate,
    };

    try {
      console.log("Called");
      console.log(
        `${urlServer}investment/portfolio/${savingInvestmentPort[0]?.Portfolio_ID}`
      );
      const response = await fetch(
        `${urlServer}investment/portfolio/${savingInvestmentPort[0]?.Portfolio_ID}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(updateInvestmentDataPortData),
          credentials: "include",
        }
      );
      if (response.ok) {
        const data = await response.json();
        console.log("done........................");
        console.log(updateInvestmentDataPortData);
        setShowCongratulatoryMessage(true);
      } else {
        const errorData = await response.json();
        console.log(errorData);
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-5">
            ...
            <div>
              ...
              <div className="flex items-center justify-center text-black text-gray-500">
                {savingInvestmentPort[0]?.RiskSpectrum}
              </div>
            </div>
            <div>
              <div className="flex items-center justify-center text-black font-bold">
              ...
              <div className="flex items-center justify-center text-black text-gray-500">
                {savingInvestmentPort[0]?.ReturnRate}
              </div>
            </div>
          </div>
        </div>
            ...
            >
              ...
              <div className="mb-4">
                ...
                <input
                  ...
                  id="first-name-input"
                  type="text"
                  placeholder={"ระดับ " + savingInvestmentPort[0]?.RiskSpectrum}
                  value={investRiskSpectrum}
                  onChange={(e) => setInvestRiskSpectrum(e.target.value)}
                />
              </div>
              <div className="mb-4">
                <label
                ...
                <input
                  ...
                  id="first-name-input"
                  type="text"
                  placeholder={savingInvestmentPort[0]?.ReturnRate + "%"}
                  value={investReturnRate}
                  onChange={(e) => setInvestReturnRate(e.target.value)}
                />
              </div>
