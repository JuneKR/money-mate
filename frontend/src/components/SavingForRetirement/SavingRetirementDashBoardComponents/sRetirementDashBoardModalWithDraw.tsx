...
> = ({ savingRetirement }) => {
  ...
  const withdrawForm = async () => {
    ...
    try {
      console.log("Called");
      const response = await fetch(`${urlServer}saving/retirement/${savingRetirement.Retirement_ID}/transaction`, {
        method: "POST",
        ...
        <Box sx={style}>
          ...
          >
            ...
            <div className="py-5 text-white font-bold text-md">
              ถอนเงินสำเร็จ! กรุณารอสักครู่และกรุณาอย่าออกจากหน้านี้....
            </div>
          </Typography>
        </Box>
      </Modal>
