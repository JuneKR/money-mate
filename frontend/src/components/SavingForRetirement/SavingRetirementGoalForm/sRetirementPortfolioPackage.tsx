...
type PackageProps = ReInvestmentData & {
...
export function SRetirementPortfolioPackage({
  dateOfBirth,
  targetAmount,
  period,
  monthlySaving,
      ...
      try {
        if (riskLevel) {
          ...
          const packageResponse = await fetch(
            `${urlServer}portfolio/package/risk-spectrum/${riskLevel}`,
            {
              credentials: "include",
            }
          );
      ...
      >
        <div>
          ...
          <div className="bg-gray-50">
            <div style={{ backgroundColor: "#1D1D41" }}>
              {portfolioPackage &&
              portfolioPackageAllocation &&
              portfolioPackageAllocation.length > 0 ? (
                <InvestmentCheckBox
                ...
                <div className="flex items-center justify-center h-full p-20">
                  <p className="text-3xl text-white">
                    ไม่มีแพ็คเกจการลงทุนตามระดับความเสี่ยงที่ท่านเลือกในขณะนี้
                  </p>
                </div>
              )}
            </div>
...
export default SRetirementPortfolioPackage;
